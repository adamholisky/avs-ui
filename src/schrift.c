...
#define STACK_FREE(var) \
	if (var != var##_stack_) avs_free(var); */

#define STACK_ALLOC(var, type, thresh, count) var = avs_calloc(sizeof(type), count);
#define STACK_FREE(var) avs_free(var);
...
static void *sc_reallocarray(void *optr, size_t nmemb, size_t size);
...
sft_loadmem(const void *mem, size_t size)
{
	...
	if (!(font = avs_malloc(sizeof *font))) {
		return NULL;
	}
	font->memory = mem;
...
static void *
sc_reallocarray(void *optr, size_t nmemb, size_t size)
{
	...
	return avs_realloc(optr, size * nmemb);
}
...
init_outline(Outline *outl)
{
	...
	outl->numPoints = 0;
	outl->capPoints = 64;
	if (!(outl->points = avs_malloc(outl->capPoints * sizeof *outl->points)))
		return -1;
	outl->numCurves = 0;
	outl->capCurves = 64;
	if (!(outl->curves = avs_malloc(outl->capCurves * sizeof *outl->curves)))
		return -1;
	outl->numLines = 0;
	outl->capLines = 64;
	if (!(outl->lines = avs_malloc(outl->capLines * sizeof *outl->lines)))
		return -1;
	return 0;
}
...
{
	avs_free(outl->points);
	avs_free(outl->curves);
	avs_free(outl->lines);
}
...
grow_points(Outline *outl)
{
	...
	if (!(mem = sc_reallocarray(outl->points, cap, sizeof *outl->points)))
		return -1;
...
grow_curves(Outline *outl)
{
	...
	if (!(mem = sc_reallocarray(outl->curves, cap, sizeof *outl->curves)))
		return -1;
...
grow_lines(Outline *outl)
{
	...
	if (!(mem = sc_reallocarray(outl->lines, cap, sizeof *outl->lines)))
		return -1;
